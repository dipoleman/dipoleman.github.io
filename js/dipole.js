...
function drawAxes() {
  ...
  ctx.lineTo(
    center_x + radius * Math.cos(Math.PI / 4) * 1.2,
    center_y - radius * Math.sin(Math.PI / 4) * 1.2
  );
  ctx.stroke();
...
function drawSoul() {
  ...
  let thoughtsCheckbox = document.querySelector(
    'input[name="options"][value="thoughts"]'
  );
  let soulVectorCheckbox = document.querySelector(
    'input[name="options"][value="soulVector"]'
  );
  let moralPlaneCheckbox = document.querySelector(
    'input[name="options"][value="moralPlane"]'
  );
  let axisCheckbox = document.querySelector(
    'input[name="options"][value="axis"]'
  );
  let boundaryCheckbox = document.querySelector(
    'input[name="options"][value="boundary"]'
  );
...
class Soul {
  ...
  drawThoughts(thoughts, thoughtsRatio, drawPoints = true) {
    if (thoughts) {
      ...
      let numThoughts =
        Math.floor(((canvasWidth * canvasHeight) / 200) * Math.random()) +
        Math.floor((canvasWidth * canvasHeight) / 200);
  ...
  drawPoint(angle, distance, color) {
    ...
    let x =
      this.center_x +
      this.radius * Math.cos((-angle * Math.PI) / 180) * distance;
    let y =
      this.center_y +
      this.radius * Math.sin((-angle * Math.PI) / 180) * distance;
  ...
  drawVector({
    ...
    this.ctx.lineTo(
      x - headlen * Math.cos(vecAngle - Math.PI / 6),
      y - headlen * Math.sin(vecAngle - Math.PI / 6)
    );
    this.ctx.moveTo(x, y);
    this.ctx.lineTo(
      x - headlen * Math.cos(vecAngle + Math.PI / 6),
  ...
  drawMoralEllipse({
    ...
    this.ctx.ellipse(
      ...
      0,
      2 * Math.PI
    ); // create an ellipse path with a width based on the length of the vector
  ...
  drawAxes() {
    ...
    this.ctx.lineTo(
      this.center_x + this.radius * Math.cos(Math.PI / 4) * 1.2,
      this.center_y - this.radius * Math.sin(Math.PI / 4) * 1.2
    );
    this.ctx.stroke();
    ...
    this.ctx.fillStyle = "black";
    this.ctx.textAlign = "center";
    this.ctx.fillText("Positive (+ve)", this.canvas.width / 2, 30);
    this.ctx.fillText("Negative (-ve)", this.canvas.width / 2, this.canvas.height - 10);
  }
